...
$(document).ready(function(){
  ...
  socket.on("response-user-is-typing", function(response) {
    let messageTyping = `<div class = "bubble you bubble-typing-gif">
      <img src="/images/chat/typing_2.gif"/>
    </div>`;

    if(response.currentGroupId){
      if(response.currentUserId !== $("#dropdown-navbar-user").data("uid")){
        let checkTyping = $(`.chat[data-chat = ${response.currentGroupId}]`).find("div.bubble you bubble-typing-gif");
      ...
      let checkTyping = $(`.chat[data-chat = ${response.currentUserId}]`).find("div.bubble you bubble-typing-gif");
      if(checkTyping.length){
        return false;
      };
  ...
  socket.on("response-user-is-not-typing", function(response){
    if(response.currentGroupId){
      if(response.currentUserId !== $("#dropdown-navbar-user").data("uid")){
        $(`.chat[data-chat = ${response.currentGroupId}]`).find("div.bubble you bubble-typing-gif").remove();
        nineScrollRight(response.currentGroupId);
      }
    }else {
      $(`.chat[data-chat = ${response.currentUserId}]`).find("div.bubble you bubble-typing-gif").remove();
      nineScrollRight(response.currentUserId);
    }
  });
});